...
The package-manager conversion preserves the complete currently resolved runtime/build package-version set, including transitive packages, not only direct dependencies. The only allowed differences are development-tool changes required to make the frozen toolchain installable and explicitly reviewed package-manager representation differences. Do not run `npm audit fix`, broad dependency updates, or formatters over runtime source.
...
| Root commands | `lint`, `test:unit`, `build:whiteboard`, `forge:lint`, `test:e2e:list`, `validate` |
...
8. Add a comparison script that reads the checked-in baseline graph and current pnpm lock. Compare importer-by-importer direct resolutions and reachable dependency edges, retaining runtime/build/dev classification; a global `name@version` union is insufficient.
9. Require every old product/build edge to resolve to the same version. The allowlist may contain only the hooks peer correction, newly added development tools, and proven package-manager topology-only differences, each with a reason. Any tldraw, React, CRA, Forge runtime, compression, Mixpanel, or transitive product edge/version difference blocks WP1.
...
7. Make `validate` execute the resolution guard, lint, unit contracts, build, resource-output validation, Forge lint, and E2E collection in that order.
8. Disable Forge CLI usage analytics through an explicit non-interactive command; do not hide Forge lint failures.
...
Expected: each command succeeds separately; `static/spa/build/index.html` exists; no runtime file was edited.
...
3. Make `forge-tunnel` verify credentials without printing them, inspect Forge identity/environment/install state, verify port 3000, build, and stop before deploy/install/start/kill in its dry run.
...
Do not print credentials or mutate Forge state during the dry run.
...
4. Give the build job exact display name `Build and Unit Test`, `contents: read`, a 15-minute timeout, Node 22.22.3, pnpm from `packageManager`, frozen install, and `pnpm validate`.
...
1. Support `workflow_call` and manual dispatch on the selected branch.
...
3. Check out the exact SHA, install frozen dependencies, run `pnpm validate`, deploy with `pnpm forge:deploy:tldraw:staging`, and upload the unchanged manifest plus generated frontend build.
...
5. The deploy job checks out and rebuilds the same tag, then uses `pnpm forge:deploy:tldraw:production`; it never installs, rewrites the manifest, parses variants, or deploys Cloudflare.
...
- manually dispatch staging only after the GitHub environment exists;
...
volta run --node 22.22.3 corepack pnpm forge:lint
...
3. Confirm `staging-tldraw` exists and has Forge credentials without exposing their values.
4. Manually dispatch staging for the branch; verify the exact SHA, generated artifact, unchanged manifest identity, and Forge deploy result.
...
7. A branch-SHA manual staging run cannot satisfy the main-push draft gate. Do not exercise draft creation before an independently authorized merge produces a successful main staging run.
